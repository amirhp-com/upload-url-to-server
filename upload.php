 ...
 * @Last modified time: 2026/08/10 20:00:00
 * @Version: 3.8.2
 */
...
define('BUILD_DATE','2026-08-10 &middot; 1405-05-19');
...
// Bulk "Save Selected to Server": download every selected FTP file onto THIS server,
// into an optional custom subfolder. Selected sub-folders are expanded recursively,
// preserving their tree under the destination.
function ftpBulkSaveToServer(){
  if(!ftpCreds||!ftpCreds.h){showToast('Connect to a server first');return;}
  var files=[],folders=[];
  document.querySelectorAll('#ftp-tbl-wrap .ftp-chk:checked').forEach(function(c){
    if(c.dataset.type==='file')files.push({path:c.dataset.path,name:c.dataset.name,size:+c.dataset.size||0});
    else folders.push({path:c.dataset.path,name:c.dataset.name});
  });
  if(!files.length&&!folders.length){showToast('No items selected');return;}
  var dest=(document.getElementById('ftp-bulk-dest')||{value:''}).value.trim().replace(/^\/+|\/+$/g,'');
  var items=files.map(function(f){return{path:f.path,name:f.name,size:f.size,folder:dest};});
  if(!folders.length){ftpRunDownloads(items);return;}
  ftpLog('Expanding '+folders.length+' selected folder(s) to download…','info');showToast('Scanning folders…');
  Promise.all(folders.map(function(fo){return ftpExpandForDownload(fo,dest);})).then(function(lists){
    lists.forEach(function(l){items=items.concat(l);});
    if(!items.length){showToast('Nothing to download — selected folder(s) are empty');return;}
    ftpRunDownloads(items);
  });
}
function ftpExpandForDownload(folder,dest){
  var fd=new FormData();fd.append('_a','ftp_tree');fd.append('_h',ftpCreds.h||'');fd.append('_port',ftpCreds.port||'21');fd.append('_u',ftpCreds.u||'');fd.append('_pw',ftpCreds.pw||'');fd.append('_method',ftpCreds.method||'ftp');fd.append('_p',folder.path);
  return fetch('',{method:'POST',body:fd}).then(function(r){return r.json();}).then(function(d){
    if(!d.ok||!d.tree){ftpLog('Could not expand "'+folder.name+'": '+((d&&d.msg)||'error'),'err');return[];}
    if(d.capped)ftpLog('"'+folder.name+'" is very large — its tree was truncated, some files may be skipped','err');
    ...
    (function walk(arr){if(!Array.isArray(arr))return;arr.forEach(function(n){
      ...
      var relDir=sub.indexOf('/')>=0?sub.replace(/\/[^/]*$/,''):'';
      var f=(dest?dest+'/':'')+folder.name+(relDir?'/'+relDir:'');
      out.push({path:n.path,name:n.name,size:+n.size_bytes||0,folder:f});
    });})(d.tree);
    return out;
  }).catch(function(e){ftpLog('Network error expanding "'+folder.name+'": '+((e&&e.message)||e),'err');return[];});
}
function ftpRunDownloads(items){
  ftpLog('Saving '+items.length+' file(s) to this server'+(items[0]&&items[0].folder?' → '+items[0].folder:'')+' …','info');
  var i=0,ok=0,fail=0;
  (function next(){
    if(i>=items.length){ftpLog('Done: '+ok+' saved'+(fail?', '+fail+' failed':''),fail?'err':'ok');showToast('Saved '+ok+' file(s)'+(fail?' · '+fail+' failed':''),fail?'':'ok');_fbLoaded=false;/* File Explorer reloads next open, showing the new files */return;}
    var it=items[i++];
    var row=ftpProgRow('↓ '+(it.folder?it.folder+'/':'')+it.name,it.size||0);
    var fd=new FormData();fd.append('_a','ftp_copy');fd.append('_h',ftpCreds.h||'');fd.append('_port',ftpCreds.port||'21');fd.append('_u',ftpCreds.u||'');fd.append('_pw',ftpCreds.pw||'');fd.append('_method',ftpCreds.method||'ftp');fd.append('_p',it.path);if(it.folder)fd.append('_folder',it.folder);
    fetch('',{method:'POST',body:fd}).then(function(r){return r.json();}).then(function(d){
      ...
      if(d.ok){ok++;ftpLog('Saved: '+it.name+(d.url?' → '+d.url:''),'ok');}else{fail++;ftpLog('Failed: '+it.name+' — '+(d.msg||'error'),'err');}
      next();
    }).catch(function(e){if(row){row.className='bulk-item err';row.querySelector('.bulk-item-ic').textContent='✗';row.querySelector('.bulk-item-meta').textContent='network error';}fail++;ftpLog('Network error: '+it.name,'err');next();});
  })();
}
  ...
  <div id="form-ftp" class="view" data-view="ftp">
    ...
    <div class="ftp-browser" id="ftp-browser-area" style="display:none">
      ...
      <div id="ftp-bulk" class="fb-bulk">
        ...
        <input type="text" id="ftp-bulk-dest" class="fb-bulk-dest" placeholder="save into folder (optional, e.g. downloads)" title="Subfolder on THIS server to download the selected items into. Leave empty for the script's folder." autocomplete="off" onclick="this.select()">
        <button class="btn btn-p btn-sm" onclick="ftpBulkSaveToServer()"><?=ph('download-simple',13)?> Save Selected to Server</button>
