 ...
 * @Last modified time: 2026/04/30 12:00:00
 * @Version: 2.0.0
 */
...
(function(){try{var m=localStorage.getItem('bsu_mode');if(m==='mitm')switchMode('mitm');}catch(e){}})();
</script>
</head>
<body>
<header class="hdr">
...
function handle_mitm_upload(){
  ...
  if(!filter_var($mitm_url,FILTER_VALIDATE_URL)||!filter_var($url,FILTER_VALIDATE_URL)||$name===''){
    ...
    flush_buffers(true);render_completion_buttons('');return;
  }
  ...
  if(!$result['ok']){
    ...
    flush_buffers(true);render_completion_buttons($path_full);return;
  }
